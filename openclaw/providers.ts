...
class SelfHostedProvider implements Mem0Provider {
  ...
  private async request(
    ...
    if (!response.ok) {
      ...
      throw new Error(`mem0 self-hosted API error ${response.status}: ${text}`);
    }
  ...
  async add(
    ...
    if (options.custom_instructions) {
      payload.custom_instructions = options.custom_instructions;
    }
    if (options.custom_categories) {
      payload.custom_categories = options.custom_categories;
    }
    if (options.infer !== undefined) payload.infer = options.infer;
    if (options.deduced_memories) {
      payload.deduced_memories = options.deduced_memories;
    }
  ...
  async getAll(options: ListOptions): Promise<MemoryItem[]> {
    ...
    const result = await this.request(`/memories?${params.toString()}`, {
      method: "GET",
    });
  ...
  > {
    const result = await this.request(`/memories/${memoryId}/history`, {
      method: "GET",
    });
    return Array.isArray(result) ? result : [];
  }
